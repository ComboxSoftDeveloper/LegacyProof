...
echo === Отчёты ===
for %%T in (net8.0 net9.0 net10.0) do (
    for %%R in (inherit covariance matrices) do (
        echo   %%T %%R
...
dotnet run -c Release -f net10.0 --no-build -- --filter *InheritBench* noasm > "%OUT%\probe_noasm.txt" 2>&1
set PROBE1=%ERRORLEVEL%
if exist "Bdn\LegacyProof.log" copy /Y "Bdn\LegacyProof.log" "%OUT%\probe_noasm.log" >nul
...
dotnet run -c Release -f net10.0 --no-build -- --filter *InheritBench* > "%OUT%\probe_asm.txt" 2>&1
set PROBE2=%ERRORLEVEL%
if exist "Bdn\LegacyProof.log" copy /Y "Bdn\LegacyProof.log" "%OUT%\probe_asm.log" >nul
ping -n 3 127.0.0.1 >nul
if exist "Bdn\*.log" del /Q "Bdn\*.log" 2>nul
...
echo.
echo   NextBench
dotnet run -c Release -f net10.0 --no-build -- --filter *InheritBench*
if errorlevel 1 goto bench_failed

echo   MethodsBench
dotnet run -c Release -f net10.0 --no-build -- --filter *SpanBench*
if errorlevel 1 goto bench_failed

echo   CreateBench
dotnet run -c Release -f net10.0 --no-build -- --filter *MatrixBench*
set MAIN=%ERRORLEVEL%

if exist "Bdn\LegacyProof.log" copy /Y "Bdn\LegacyProof.log" "%OUT%\bench.log" >nul
